...
----------|---------
    ...
    -f    | 文件 指定一个可选的配置文件，默认情况下，tmux会从/etc/tmux.conf中加载系统配置文件，如果这个文件存在的话，
          | 然后会尝试查找用户的配置文件 ~/.tmux.conf-配置文件是一个tmux命令集合，其中的命令在服务器第一次启动时按顺序执行的。
          | tmux会在服务器进程启动之后加载一次配置文件。 "source-file"命令可以用来在稍后加载一个文件。
