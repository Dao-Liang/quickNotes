...
#### shell参数扩展 ####
    ...
    ${!prefix@}
        ...
        扩展为具有该前缀的变量名称，使用IFS特殊变量的第一个字符分割，当使用@时，扩
...
##### 模式匹配 #####
	...
	其引起来，特殊的模式字符具有一下含义:
		...
		\*  匹配任何的字符串，包括空字符。当globstar 选项生效时，‘\*’被用在文件名扩展上下文中，将两个邻
...
### shell重定向 ###
	在命令执行之前，它的输入和输出可能使用shell解释的特殊的符号来进行重定向。重定向允许文件
	的占用符复制，打开，关闭，引用向不同文件，也可以改变命令读取或写入的文件。
	重定向可能也会被用来修改当前shell执行环境中的文件占用符。下面的重定向操作符可能在简单命令
	之前或出现在命令中以及命令之后。重定向会按照他们出现的顺序，按照从左到右的顺序执行。

	每个重定向可以使用文件描述符前缀数字可能使用{varname}的形式进行替换。在这样的情况下，对于
	每个重定向操作符，除了>&-和 <&-之外，shell会分配一个大于10的文件描述符将其赋值给{varname}.
	如果>&-或<&-作为{varname}的前缀，varname定义了关闭的文件描述符值。

	在下面的描述中，如果忽略数字被忽略，并且重定向操作符的第一个字符是'<',那么重定向就会引用
	标准输入(文件描述符0). 如果重定向操作符的第一个字符为'>'，那么重定向就会引用标准输出(文件
	描述符1).

	在下面描述的重定向操作符后面跟随的单词，除非提到，会经历括号扩展，波浪号扩展，参数扩展，命令扩展，
	算术扩展，引号移除，文件名扩展以及单词分割。如果其被扩展为多于一个单词，bash就会报告一个错误。

	注意，重定向的顺序是有很重要的意义的，例如命令
	ls > dirlist 2>&1
	将标准输出和标准错误定向到dirlist中，而命令
	ls 2>&1 > dirlist
	只会将标准输出定向到文件案dirlist中，因为标准错误在标准输出重定向到dirlist之前会创建一个标准输出的
	复制。

	bash会对重定向中的一些文件名进行特殊处理，会在下面进行描述处理：
		/dev/fd/fd 		如果fd是一个合法的整数，文件描述符被复制
		...
		/dev/tcp/host/port	如果host是一个合法的主机名或者IP地址，并且Port是一个整数port号或服务名，
							bash会尝试打开对应的TCP套结字

		/dev/udp/host/port	如果host是一个合法的主机名或者IP地址，并且Port是一个整数port号或服务名，
							bash会尝试打开对应的UDP套结字

	创建或打开文件爱你失败会导致重定向失败。

	使用大于9的文件描述符应该注意，因为他们可能会与shell内部使用的文件描述符产生冲突。
