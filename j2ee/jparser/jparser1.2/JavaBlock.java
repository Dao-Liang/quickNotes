...
// private final List<JavaBlock> anonymousBlocks;
	...
	private String targetMethodParametername;

	private String targetImportTypeName = "";

	private ClassContainer clazz;
...
// this.anonymousBlocks = new LinkedList<JavaBlock>();
	...
	public void typeBlockParse() {
		...
		// class has NOT_SUPPORT annotation
		if (this.isNonTransactionalBlock()) {
			clazz.setNotSupport(true);
		}
...
// JavaStatementType.SIMPLE_STATEMENT, sb.toString().split("\\s+"));
					...
					fieldP = Pattern.compile(this.getTargetImportTypeName() +
							"\\s+([\\w\\d_]+)\\s+");
					Matcher fieldM = fieldP.matcher(sb.toString());
					if (fieldM.find()) {
						System.err.println("------Target Field declaration: " +
								fieldM.group(1));
						this.setTargetFieldname(fieldM.group(1));

					} else {
						if (this.getTargetImportTypeName().endsWith("Bean")) {
							fieldP = Pattern.compile(this.getTargetImportTypeName().substring(
									0, this.getTargetImportTypeName().length() - 4) +
									"\\s+([\\w\\d_]+)\\s+");

							fieldM = fieldP.matcher(sb.toString());
							if (fieldM.find()) {
								System.err
										.println("------Target Interface type Declaration: " +
												fieldM.group(1));
								this.setTargetFieldname(fieldM.group(1));
							}
						}
			...
			if (!isCommonStatement && isMethodBlock) {
				...
				if (!isInMethodStatement && methodBracketCount == 0) {
					...
					if (this.isNonTransactionalBlock) {
						...
						methodBlock.setTargetImportTypeName(this.getTargetImportTypeName());
						if (this.getTargetFieldname() != null) {
							methodBlock.setTargetFieldname(this.getTargetFieldname());
						}
						methodBlock.setClazz(clazz);
						methodBlock.parseBlock();
...
// blockContent));
	...
	/**
	 * 
	 * used to parse the target type parameter declaration for this method for
	 * later use
	 */
	private void parseMethodDeclarationStatement(String methodDeclaration) {

// System.out.println("------Method declaration: " + methodDeclaration);
		Pattern methodDeclarationP = Pattern.compile("\\s*([\\w\\d_]+)\\s*\\(");
		Matcher methodDeclarationM = methodDeclarationP.matcher(methodDeclaration);
		if (methodDeclarationM.find()) {

			String methodName = methodDeclarationM.group(1);
			MethodContainer method = new MethodContainer();
			method.setMethodName(methodName);
			if (this.isNonTransactionalBlock()) {
				method.setNotSupport(this.isNonTransactionalBlock());
			}
			clazz.addMethod(method);
		}

		Pattern parameterP = Pattern.compile(this.getTargetImportTypeName() +
				"\\s+([\\w\\d_]+)\\s+[,\\)]");
		Matcher parameterM = parameterP.matcher(methodDeclaration);
		if (parameterM.find()) {
			targetMethodParametername = parameterM.group(1);
		}
...
// // test if method has the target type field declaration
// if (this.getTargetFieldname() != null) {
// System.err.println("------Method has Field variable defined: " +
// this.getTargetFieldname());
// }

		// test if method is NonTransactional
		if (this.isNonTransactionalBlock()) {
// System.err.println("------Method is NonTransactional!!!");
		}
...
// blockContent));
		...
		for (int i = 0; i < blockContent.length; i++) {
			...
			if (!currentToken.equals("{") && !isInMethod) {
				...
				this.parseMethodDeclarationStatement(sb.toString());
				sb = new StringBuilder();
				isInMethod = true;
				continue;
			}
			...
			if (isCommonStatement && !isInIfBlock && !isInForBlock && !isInDoWhileBlock &&
				 ...
				 * 
				 * 1. target import type variable declaration
				 * 
				 * 2. target variable invocationTODO
				 * 
				 * TODO
				 * 
				 * 
 ...
 */
	...
	public void setTargetMethodParametername(String parameterName) {

		this.targetMethodParametername = parameterName;
	}

	public void setTargetImportTypeName(String typeName) {

		this.targetImportTypeName = typeName;
	}
	...
	public String getTargetMethodParametername() {

		return this.targetMethodParametername;
	}

	public String getTargetImportTypeName() {
		return this.targetImportTypeName;
	}
	 ...
	 *            The clazz to set.
	 */
	public void setClazz(ClassContainer clazz) {
		this.clazz = clazz;
	}
